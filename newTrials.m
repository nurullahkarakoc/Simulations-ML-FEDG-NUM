...
x2=Rtot*ones(N,1); % Initial values of allocations for 2-layer
x3=Rtot*ones(S,O); % Initial values of allocations for 3-layer
x3u=Rtot/N*ones(S,O); % Initial used values of allocations for 3-layer
lambda=5; % Initial lambda
...
for i=1:Niter
    flag=0;
    if(mod(i,T)==0 & i==1) % renew parameters each time slot
        w=rand(N,1)+1;
